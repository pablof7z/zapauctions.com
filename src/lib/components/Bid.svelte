<script>
    ...
    function zapContent(content) {
        ...
        if (content.match(/(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|jpeg|gif|png)/g)) {
            return `<img src="${content}" class="max-h-32" />`;
...
<div
  ...
  cursor-pointer md:mb-4 md:rounded md:shadow border-b-gray-300 dark:border-b-gray-800 border-b
  items-center
  justify-between
...
"
>
    <div class="w-full flex flex-row gap-1 justify-between py-4">
        <div class="w-2/5 flex flex-row items-center gap-4">
            <Avatar klass="m-2 w-16 h-16 ring-8 ring-purple-1000" pubkey={zap.sender} />
            <div class="font-bold text-xl hidden sm:block overflow-hidden">
            ...
            <div class="text-5xl font-black text-center justify-center flex flex-row items-center">
                ...
                <span class="flex flex-col items-center">
                    <span class="text-purple-900 dark:text-purple-500">
                        {formatSatoshis(zap.amount, { tryGrouping: true, justNumber: true })}</span
                    >
                    <span class="text-lg text-gray-600 dark:text-gray-300 font-black uppercase"
                        >{formatSatoshis(zap.amount, { tryGrouping: true, justUnit: true })}</span
                    >
                </span>
            </div>
        </div>
    ...
    {#if zap.content}
      ...
      "
        >
            {#if zapContent(zap.content)}{@html zapContent(zap.content)}{/if}
        </div>
    {/if}
</div>
